...
struct Color
{
	...
	void ApplyDiffuse(float& Diffuse);
	void ApplySpecular(float& Specular);
	void AppplyMaterial(Material& Mat);
	void Clamp(const float Min = 0.0f, const float Max = 1.0f);
	void BlendColor(const Color vb,float Alpha);
...
struct RayLine
{
	...
	RayLine(const Position3& Start, const Vector3& Vec) :StartPos(Start), Ray(Ray.Normalized()) {};

	void MakeRay(const Position3& Start, const Vector3& End)
	{
		StartPos = Start;
